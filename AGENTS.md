...
- **Deploy migrations.** Pending Drizzle migrations run at server boot via `instrumentation.ts`. Databases originally created with `db:push` are baselined automatically so only new migrations apply.
