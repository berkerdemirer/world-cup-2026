...
- **Deploy migrations.** Pending Drizzle migrations run at server boot via `instrumentation.ts` (not during `next build`), because Vercel/Neon expose `DATABASE_URL` at runtime only. Use `pnpm db:migrate` manually for local/CI.
